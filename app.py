...
def send_admin_email(subject, content):
    try:
        ...
        server.starttls()
        server.login('noreply@yourdomain.com', 'your_email_password')  # Use environment variables in production!
...
@app.route('/delete/int:<int:index>', methods=['POST'])
def delete(index):
...
def login():
    if request.method == 'POST':
        ...
        if email == 'admin@example.com' and password == 'admin123':  # Replace with secure credentials!
            session['admin'] = True
            return redirect('/admin-dashboard')
        flash('Invalid login')
