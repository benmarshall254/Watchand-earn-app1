...
@app.route('/dashboard') def dashboard(): if not session.get('admin'): flash("Unauthorized access.") return redirect(url_for('admin_login')) return render_template('dashboard.html', videos=videos, users=users, withdrawals=withdrawals, visitors=visitor_count, settings=settings, rules=reward_rules)
...
@app.route('/profile') def profile(): if 'firebase_user' not in session: flash("Please log in to view your profile.") return redirect(url_for('login'))
